...
/*
 * I havent implemented the binary heap data structure myself. It was taken from: 
 
    http://content.gpwiki.org/index.php/C_sharp:BinaryHeapOfT
 
 * with just few modifications to work as max binary heap;
 */
...
public class BinaryHeap<T> : ICollection<T> where T : IComparable<T>
{
    ...
    {
        if (_sorted) return;
        var sortedArray = from s in _data
                          orderby s descending
                          select s;
        _data = sortedArray.ToArray();
        _sorted = true;
    }
