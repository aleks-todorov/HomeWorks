...
public class Ninja : Character, IFighter, IGatherer
{
    ...
    public int GetTargetIndex(List<WorldObject> availableTargets)
    {
        availableTargets = availableTargets.OrderBy(t => t.HitPoints).ToList();
        for (int i = 0; i < availableTargets.Count; i++)
        {
            if (availableTargets[i].Owner != this.Owner && availableTargets[i].Owner != 0)
            {
...
class Program
{
    static UpgradedEngine GetEngineInstance()
    {
    ...
    {
        //I couldnt find a way to do it without changing type to UpgradedEngine
        UpgradedEngine engine = GetEngineInstance();
