    ...
    public class AdvancedFigureController : FigureController
    {
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "area":
                    {
                        var currentAsIAreaMesurable = this.currentFigure as IAreaMeasurable;
                        if (currentAsIAreaMesurable != null)
                        {
                            Console.WriteLine("{0:0.00}", currentAsIAreaMesurable.GetArea());
                        }
                        else
                        {
